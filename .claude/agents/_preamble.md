...
| FP-006 | AI 응답 mocking (Bedrock 미호출, 정형 stub 반환) | code-generator-ai, reviewer (cat 7), ai-smoke Check 2/3 |
| FP-007 | `@aws-sdk/client-bedrock-runtime` 직접 import | code-generator-ai, reviewer (cat 7), ai-smoke Check 1 |
| FP-008 | `process.env.BEDROCK_MODEL_ID` 환경변수 fallback | code-generator-ai, reviewer (cat 10), ai-smoke Check 8 |
| FP-009 | 허용된 3개 외 모델 ID 사용 | code-generator-ai, reviewer (cat 10), ai-smoke Check 7 |
| FP-010 | `index.ts` barrel export, 파일당 `export default` 2개 이상 | reviewer (cat 7) |
| FP-011 | `.pipeline/state.json` 직접 Write/Edit/sed/jq\|sponge/리다이렉트/`node -e`/`python -c`/`fs.writeFileSync` 등 우회 — 모든 state 변경은 `checkpoint.mjs` 서브커맨드 경유 | settings.json PreToolUse hook(Write/Edit/Bash 패턴 차단), reviewer (cat 7) |
...
|---|---|---|---|
| opus | max | architect, aws-architect, aws-deployer, code-generator-* | 복잡 추론·코드 합성·아키텍처 의사결정 |
...
**적용 우선순위**: `requirements-analyst`, `architect`, `spec-writer-*`, `code-generator-*` (산출물이 큰 에이전트). 짧은 정형 작업(`git-manager`, `brief-composer`)은 마크다운만으로 충분.

이 권고는 **각 에이전트가 점진적으로 본문을 리팩터링**할 때 적용하며, 일괄 변경은 강제하지 않는다.
