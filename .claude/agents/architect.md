...
> **두 범위 게이트의 관계 (requirements-analyst ↔ architect)**: requirements-analyst는 **요구사항 층위**에서 P0 FR이 15개를 초과하면 `phase-1/phase-2-requirements.json`으로 **분할**한다 (전체 프로토타입을 단계로 쪼갬). architect는 **설계 층위**에서 그 결과로 들어온 FR을 라우트로 펼친 뒤 라우트가 20개를 초과하면 상위 20개만 v{N} 범위로 **컷오프**한다 (단일 단계 내 라우트 상한). 즉 15개는 "단계 분할 트리거", 20개는 "단계 내 라우트 상한"으로 서로 다른 층위이며, 둘 다 독립적으로 작동한다. requirements-analyst가 이미 phase로 분할했다면 architect는 해당 phase의 requirements.json만 입력으로 받으므로 라우트 20개를 넘기 어렵다 — 그래도 넘으면 위 fallback이 추가 안전망이다.
