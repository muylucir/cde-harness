...
1. **Read**: requirements.json, domain-context.json (있으면) + 스킬 호출 (`cloudscape-design`, `ascii-diagram`, `mermaid-diagrams`)
2. **Write**: `architecture.json` — `metadata`, `pages[]`, `api_routes[]` 포함
3. **Edit**: `architecture.json` — `hooks[]`, `types[]`, `shared_components[]`, `layout_components[]`, `contexts[]`, `requirements_coverage` 추가
4. **Write**: `architecture.md` — 컴포넌트 트리 + 데이터 플로우 + 커버리지 매트릭스
