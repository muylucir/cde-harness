...
- Read: requirements.json, domain-context.json (있으면)
- 스킬 호출: `cloudscape-design`, `ascii-diagram`, `mermaid-diagrams`
- 읽은 후 요약을 출력하고 **멈춘다**: "입력 읽기 완료. FR {N}건, 페이지 {M}개 설계 예정. Primary persona: {역할}."
- **이 턴에서 Write/Edit를 호출하면 안 된다.**

### 턴 2: architecture.json 전반부
- Write: `architecture.json` — `metadata`, `pages[]`, `api_routes[]` 포함

### 턴 3: architecture.json 후반부
- Edit: `architecture.json` — `hooks[]`, `types[]`, `shared_components[]`, `layout_components[]`, `contexts[]`, `requirements_coverage` 추가

### 턴 4: architecture.md
- Write: `architecture.md` — 컴포넌트 트리 + 데이터 플로우 + 커버리지 매트릭스
