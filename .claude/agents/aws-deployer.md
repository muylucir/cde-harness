...
2. 휴면 `agent-runtime/` 패키지 확인·활성화:
   - **형태 판별 (Rule 14.3 층위 2)**: `agent-runtime/src/index.ts`(단일 진입점, 9-A) 또는 `agent-runtime/{orchestrator,...}/src/app.ts`(per-agent A2A 진입점, 9-B) 중 어느 형태인지 확인. 둘 다 없으면 code-generator-ai를 먼저 실행하라고 보고 후 중단(진입점을 여기서 새로 만들지 않는다).
   - `cd agent-runtime && npx tsc --noEmit`로 컴파일 통과 확인(코어 이식성 검증).
   - **단일 진입점(9-A)이면** 단일 Runtime으로 배포한다. **per-agent(9-B)면** 아래 단계 5-B로 오케스트레이터 + 도메인 런타임을 각각 배포한다. 어느 경우든 진입점은 code-generator-ai가 이미 만든 것을 **활성화**할 뿐 — 코어/진입점을 다시 쓰지 않는다(코어 미수정 원칙).
...
- 이어서 `agent-runtime/orchestrator`를 배포하고, `.env.local`의 도메인별 `A2A_URL_*`를 위에서 확보한 도메인 런타임 ARN/URL로 채운다 → 오케스트레이터의 `A2ADelegation` 경로 활성화(미설정 시 InProcess, 코드는 동일).
- **코어 미수정 원칙 유지**: 위임 분기(`A2A_URL_*` 유무 → `A2ADelegation|InProcessDelegation`)는 code-generator-ai가 규칙 8.4로 이미 코드에 넣었다. aws-deployer는 `A2A_URL_*` env만 채운다 — `src/lib/ai/` 코어와 `agent-runtime/` 진입점을 다시 쓰지 않는다.
...
**중요**:
- `code-generator-ai`가 생성한 `src/lib/ai/` 코어와 `agent-runtime/` 진입점은 **새로 작성하지 않는다** — 활성화·배포·환경변수(`GATEWAY_URL`/`GATEWAY_AUTH`/`A2A_URL_*`/`AI_RUNTIME`) 배선만 한다. (진입점/seam 신규 작성은 code-generator-ai 규칙 8/9의 책임)
