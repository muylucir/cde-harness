...
- **표준 도구**: `@aws/agentcore` npm CLI (`npm install -g @aws/agentcore`) + `bedrock-agentcore` SDK (pip) + `agentcore.json` 설정
- CLI 워크플로우: `agentcore create` → `agentcore dev`(로컬 테스트) → `agentcore deploy`(CDK 기반 프로비저닝) → `agentcore status`(Runtime ARN 확인)
- `BedrockAgentCoreApp` 래핑 패턴: `from bedrock_agentcore.runtime import BedrockAgentCoreApp` + `@app.entrypoint def invoke(payload)` (Strands 에이전트 → AgentCore 배포용)
...
│   ├── agentcore.json                # @aws/agentcore CLI 설정 (agentcore create가 생성)
│   ├── main.py                       # BedrockAgentCoreApp 래퍼 (@app.entrypoint def invoke)
│   └── requirements.txt              # bedrock-agentcore SDK 등 (CodeZip은 Docker 불필요)
...
1. `npm install -g @aws/agentcore` (Node 20+) + `bedrock-agentcore` SDK(pip) + AWS CDK 사전 설치 확인
2. `src/lib/ai/` 에이전트를 `infra/agentcore/` 진입점으로 래핑:
   ```python
   from bedrock_agentcore.runtime import BedrockAgentCoreApp
   app = BedrockAgentCoreApp()

   @app.entrypoint
   def invoke(payload):  # payload = JSON dict (구 `def my_agent(prompt: str)` 아님)
       ...

   if __name__ == "__main__":
       app.run()
   ```
3. `agentcore create` → `agentcore.json` 스캐폴딩 생성 (region/role/모델/framework 설정은 이 파일에서)
4. `agentcore dev` → 로컬 hot-reload 테스트 (선택)
5. `agentcore deploy` → CDK 기반으로 Runtime/ECR/IAM 자동 생성 (CodeZip 기본은 Docker 불필요)
...
8. Next.js `/api/chat` route.ts를 AgentCore 호출로 교체 (해당 시)

**중요**:
- `code-generator-ai`가 생성한 `src/lib/ai/` 파일은 **수정하지 않는다**. 배포용 래퍼만 `infra/agentcore/`에 작성.
