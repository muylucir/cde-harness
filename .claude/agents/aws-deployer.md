---
name: aws-deployer
description: "aws-architecture.json을 기반으로 CDK TypeScript 코드를 생성하고 배포한다. 데이터 레이어(Polyglot Ports & Adapters, Rule 12)는 code-generator-backend가 이미 AWS SDK/PG 한 벌로 생성했으므로 수정하지 않고 endpoint env만 전환하며, 시드 데이터를 마이그레이션한다."
model: opus
effort: max
color: pink
allowedTools:
...
CDK TypeScript 인프라를 생성/배포하는 에이전트이다. 데이터 레이어는 code-generator-backend가 이미 Polyglot Ports & Adapters(Rule 12)로 생성했으므로 **수정하지 않고**, 로컬→prod 전환은 endpoint env(`AWS_ENDPOINT_URL`/`DATABASE_URL`)로만 한다(Vision B).
...
- **Gateway 타겟 배선 + 아웃바운드 auth**: `references/gateway.md`(MCP/Lambda/OpenAPI 타겟 등록 + 아웃바운드 인증 매트릭스), Identity 흡수는 `references/identity.md` 참조. 이 프로토타입의 도구 Gateway seam(`GATEWAY_URL`)을 실제 AgentCore Gateway 엔드포인트로 전환하는 데 사용(Step 4.6).
...
- 현재 `src/` 코드 — `src/lib/db/`의 포트/어댑터/`createRepositories.ts`(Rule 12, 수정 안 함), 타입, 시드 데이터
...
> **코어 미수정 원칙 (이 절 전체에 적용, Rule 14)**: `src/lib/ai/` 코어와 code-generator-ai가 규칙 8/9로 만든 휴면 `agent-runtime/` 진입점(Express `/ping`+`/invocations`, ARM64 Dockerfile)을 **새로 쓰지 않는다.** aws-deployer는 이미 있는 것을 **활성화·배포·env 배선**(`GATEWAY_URL`/`GATEWAY_AUTH`/`A2A_URL_*`/`AI_RUNTIME`)만 한다. 진입점/seam 신규 작성은 code-generator-ai의 책임. 아래 모든 Step은 이 원칙을 전제로 하며 매번 반복하지 않는다.
> **언어 일치**: AI 코어가 TS이므로 AgentCore 진입점도 TS(Express 컨테이너 BYO). Python `BedrockAgentCoreApp`/`main.py`는 Python 에이전트 전용이라 TS 코어를 감쌀 수 없다 — 만들지 않는다.
...
2. 휴면 `agent-runtime/` 패키지 확인·활성화:
   - **형태 판별 (Rule 14.3 층위 2)**: `agent-runtime/src/index.ts`(단일 진입점, 9-A) 또는 `agent-runtime/{orchestrator,...}/src/app.ts`(per-agent A2A 진입점, 9-B) 중 어느 형태인지 확인. 둘 다 없으면 code-generator-ai를 먼저 실행하라고 보고 후 중단.
   - `cd agent-runtime && npx tsc --noEmit`로 컴파일 통과 확인(코어 이식성 검증).
   - **단일 진입점(9-A)이면** 단일 Runtime으로 배포. **per-agent(9-B)면** 아래 5-B로 오케스트레이터 + 도메인 런타임을 각각 배포.
...
**5-B. per-agent A2A 멀티 런타임 배포 (9-B 형태일 때만 — Rule 14.3 층위 2)**:
- `agent-runtime/{domain-id}` 도메인 런타임을 **먼저** 각각 컨테이너→ECR→`agentcore create`/`deploy`로 배포(`agentcore.json`의 `protocol: "A2A"`). `agentcore status`로 각 도메인 Runtime ARN/엔드포인트를 확보한다.
- 이어서 `agent-runtime/orchestrator`를 배포하고, `.env.local`의 도메인별 `A2A_URL_*`를 위에서 확보한 도메인 런타임 ARN/URL로 채운다 → 오케스트레이터의 `A2ADelegation` 경로 활성화(미설정 시 InProcess, 코드는 동일). 위임 분기(`A2A_URL_*` 유무 → `A2ADelegation|InProcessDelegation`)는 code-generator-ai가 규칙 8.4로 이미 코드에 넣었다 — aws-deployer는 env만 채운다(위 코어 미수정 원칙).

### Step 4.6: Gateway 타겟 배선 (도구 Gateway seam → live, leaf 도구가 있을 때만)

도구 Gateway seam(`GATEWAY_URL`)을 mock에서 실제 AgentCore Gateway로 전환한다. `bedrock-agentcore-guide`의 `references/gateway.md`(MCP/Lambda/OpenAPI 타겟 + 아웃바운드 auth 매트릭스)를 참조한다. **코어는 미수정** — leaf 도구는 이미 `McpClient` 포트 뒤에 있으므로 Gateway 엔드포인트만 채우면 같은 데이터가 Gateway 경로로 흐른다.

1. AgentCore Gateway 생성 + 타겟 등록: 각 leaf 도구의 백엔드 종류에 맞춰 `agentcore add gateway`/`gateway-target`으로 MCP/Lambda/OpenAPI 타겟을 등록한다. 와이어 도구명은 `{target}___{tool}` 프리픽스 규약을 따른다(code-gen-ai의 `gateway-client.ts`가 이 프리픽스를 벗기고/붙인다).
2. **아웃바운드 auth (= Identity 흡수, Rule 14.5)**: leaf 도구가 외부 백엔드 자격증명을 필요로 하면(`ai-internals.json.tools[].requires_outbound_auth: true`, `auth_via: "gateway"`), Gateway의 `credentialProviderConfigurations`(IAM/OAuth 2LO·3LO/API key)로 토큰을 주입한다. 도구 코드는 토큰을 만지지 않는다. `auth_via: "direct"`인 도구만 code-gen-ai가 만든 `CredentialProvider` 주입 seam을 사용한다 — 그 경우 mock 토큰을 AgentCore Identity 토큰으로 교체한다.
3. **env 교체**: `.env.local`의 `GATEWAY_URL`을 배포된 Gateway MCP 엔드포인트로, 인바운드 토큰이 필요하면 `GATEWAY_AUTH`/`GATEWAY_TOKEN`을 설정한다. `GATEWAY_URL` 미설정이면 코어는 그대로 mock 클라이언트로 동작한다(전환은 env 교체뿐).
4. leaf 도구가 없으면(단순 추론/요약 데모) 이 단계는 건너뛴다.

**중요** (위 "코어 미수정 원칙"·"언어 일치"는 이 절 첫머리 blockquote 참조 — 반복하지 않음):
- **이중 seam 전환은 코어 0줄 수정 + env 교체 + 실물 기동**: Gateway 뒤 실제 타겟과 (9-B면) 도메인 런타임이 떠 있어야 동작한다. "env만 바꾸면 프로덕션 완성"이 아니다(Rule 14.6) — 보고에서 이 선을 넘지 않는다.
...
1. `npm run build` — 실제 AWS endpoint env로 Next.js 빌드 성공
2. 로컬 미러(ministack) endpoint env(`AWS_ENDPOINT_URL`/compose `DATABASE_URL`)로도 빌드 성공 — 코드 동일, env만 차이(Vision B)
...
- [ ] **데이터 레이어 코드 미수정 (Vision B, Rule 12)** — `src/lib/db/`의 포트/어댑터/`createRepositories.ts`를 건드리지 않았고, 전환은 endpoint env(`AWS_ENDPOINT_URL` 제거 / `DATABASE_URL`을 Aurora로)뿐. `check-repository-naming.mjs`(sub-check [B]) 통과
- [ ] `npm run build` 성공 (실제 AWS endpoint env)
- [ ] 시드 데이터 마이그레이션 완료
- [ ] deploy-log.json + migration-log.json 생성됨
- [ ] **(AgentCore enabled + leaf 도구 있을 때) Gateway 타겟 배선 + `GATEWAY_URL`/`GATEWAY_AUTH` env 교체 완료** (Step 4.6, 코어 미수정)
- [ ] **(per-agent 9-B일 때) 도메인 런타임 배포 + `A2A_URL_*` env 채움 완료** (Step 5-B, 코어 미수정)
- [ ] **`src/lib/ai/` 코어와 `agent-runtime/` 진입점을 수정하지 않았는가** (활성화·env 배선만 — Rule 14 코어 불변)
...
- 배포된 AWS 리소스 목록
- 수정/생성된 파일 수
- 로컬↔prod 전환 방법 (endpoint env: 로컬 ministack `AWS_ENDPOINT_URL`/compose `DATABASE_URL` ↔ prod 실제 AWS/Aurora — 코드 미수정, Rule 12 Vision B)
