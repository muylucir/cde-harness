...
본 에이전트는 sonnet/medium으로 동작하므로 이미지 다발 입력 시 vision 토큰이 폭발할 수 있다. 다음 임계 초과 시 사용자에게 정리 권고 후 진행 여부를 묻는다 (자동 진행 금지):

- **이미지 개수**: 8장 초과 (`.png`, `.jpg`, `.jpeg`, `.svg`, `.webp` 합계)
- **단일 이미지 크기**: 5 MB 초과
- **PDF 페이지**: 20페이지 초과 (시스템 제약)
- **총 raw/ 크기**: 50 MB 초과

가드 메시지 예:
```
입력 자료가 많습니다 (이미지 12장, 총 38 MB).
sonnet 모델로 정확한 분석이 어려울 수 있습니다.
다음 중 선택:
  (a) 핵심 자료만 남기고 정리 (권장)
  (b) 그대로 진행 (분석 정확도 저하 가능)
  (c) opus 모드로 escalate (모델을 직접 변경하려면 frontmatter 수정)
```

가드 발동 사실은 산출물 `## Notes` 섹션에 기록한다.
...
3. **Write**: `source-analysis.md` + `manifest.json` + `clarifications.md` (아래 "clarifications 생성 트리거" 조건 중 **하나라도** 해당하면 **필수 생성**)
...
- [ ] 트리거 1건 이상 해당하면 `.pipeline/input/clarifications.md`를 생성했는가 (Conflicts/Assumptions 섹션만으로 대체하지 않음)
...
| 구분 | 대상 | 출력 위치 |
|------|------|----------|
| **Conflicts** | 소스 간 모순 OR 하네스 규약 vs 고객 의도 충돌 (예: React+Vite vs Next.js). **하네스 규약이 이기므로 고객 질문 불필요**. 단순 기록용 | `customer-brief.md` → `## Conflicts` |
| **Assumptions** | clarifications 답변이 없거나 건너뛴 경우에만 사용. 추론 근거와 함께 기본값을 기록 | `customer-brief.md` → `## Assumptions` |
| **Clarifications** | **고객에게 직접 물어야 답이 나오는 누락/모호 정보**. SLA 수치, 데이터 규모, 범위, 우선순위 등 | `.pipeline/input/clarifications.md` (별도 파일) |

### clarifications 생성 트리거 (하나라도 해당하면 **필수** 생성)

분석 결과 아래 중 **하나 이상**이 감지되면 `.pipeline/input/clarifications.md` 파일을 반드시 생성한다. "Conflicts 섹션에 적었으니 충분하다"라고 판단하고 건너뛰는 것은 **금지**한다.

1. **정량 수치 누락 NFR**: "빠른 응답", "대용량 처리" 등 수치 없는 비기능 요구사항 (예: P95 latency, RPS, 동시 접속자 수 미정)
2. **데이터 규모/범위 미정**: 합성 데이터 건수, 대상 지역/국가/언어, 시계열 기간 등이 원본에 없음
3. **우선순위 모호**: 요구사항이 병렬로 나열되어 Phase 1/2 분리 기준 없음
4. **페르소나 추론 의존**: 사용자 역할/기술 수준/사용 빈도가 원본에 명시되지 않아 `requirements-analyst`가 도메인 컨텍스트로 보완해야 할 때
5. **외부 의존성 불명**: 연동 대상 시스템, 인증 방식, 기존 DB 스키마 등 이름만 언급되고 세부 내용 없음
6. **성공 지표 부재**: "프로토타입으로 무엇을 보여주면 성공인가"가 명확하지 않음

### 절차

1. raw 분석 완료 후 위 6개 트리거를 **매번** 점검한다
2. 1건이라도 해당하면 즉시 `.pipeline/input/clarifications.md`를 생성한다
3. "Conflicts" 또는 "Assumptions"에 적었더라도 고객 확인이 필요한 것이면 clarifications.md에도 **반드시** 중복 기재한다 (clarifications가 질문의 단일 소스)
4. 완료 보고 시 "확인 필요 항목 N건 → clarifications.md 작성됨" 명시
