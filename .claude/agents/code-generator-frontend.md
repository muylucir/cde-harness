...
- proxy(_preamble §11)가 세팅한 `x-user-id`/`x-user-roles` 헤더를 RootLayout에서 Context로 내려보내기
...
0. **금지 패턴 (위반 시 reviewer가 P0 반려)**: `any` 타입, `@ts-ignore`/`@ts-nocheck`, barrel export(`index.ts`로 재export), Pages Router(`pages/` 디렉터리), `data?.results`/`data?.data` 등 envelope 변형 사용. 위반은 ESLint도 차단하지만 codegen 시점부터 회피한다. **FP-001~FP-011은 담당 범위 내 모든 생성 파일에 예외 없이 적용된다 — 첫 파일만이 아니라 전체 범위이다.**
