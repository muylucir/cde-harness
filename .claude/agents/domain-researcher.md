...
### 턴 1: 브리프 읽기 + 키워드 추출 (Write/Edit 금지)
- Read: customer-brief.md
- 산업, 서브도메인, 핵심 엔티티, 사용자 역할 추출
- 읽은 후 요약을 출력하고 **멈춘다**: "브리프 분석 완료. 산업: {X}, 서브도메인: {Y}. 웹 리서치를 시작합니다."
- **이 턴에서 Write/Edit를 호출하면 안 된다.**

### 턴 2: 웹 리서치 (카테고리 2a~2c)
- 워크플로우, KPI/용어, 유사 제품 검색

### 턴 3: 웹 리서치 (카테고리 2d~2f) + 포화 판정
- 규제, 데이터 모델, 사용자 역할 검색 + 종료 조건 확인

### 턴 4: domain-context.json 작성
### 턴 5: domain-context.md 작성
