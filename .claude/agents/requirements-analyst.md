...
### 턴 1: 입력 읽기 + 스킬 호출 (Write/Edit 금지)
- Read: customer-brief.md, domain-context.json (있으면)
- 스킬 호출: `cloudscape-design` (패턴 매핑용)
- 읽은 후 요약을 출력하고 **멈춘다**:
  ```
  입력 읽기 완료.
  - 고충 사항: {N}건
  ...
  - 요구사항 후보: {K}건
  - 도메인 제안: {있음/없음}
  다음 턴에서 requirements.json 전반부를 작성합니다.
  ```
- **이 턴에서 Write/Edit를 호출하면 안 된다.**

### 턴 2: requirements.json 전반부
- Write: `requirements.json` — `metadata`, `pain_points[]`, `personas[]`, `user_stories[]` 포함

### 턴 3: requirements.json 후반부
- Edit: `requirements.json` — `functional_requirements[]`, `non_functional_requirements[]`, `data_model`, `pages[]`, `sla_definitions`, `assumptions[]`, `out_of_scope[]` 추가

### 턴 4: requirements.md
- Write: `requirements.md` — 한국어 마크다운 (페르소나 테이블, US 테이블, FR 요약, 가정사항)
