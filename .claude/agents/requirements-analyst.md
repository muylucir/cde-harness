...
10. **검증**
    ...
    - 범위가 너무 큰 경우 (P0 FR이 15개 초과), 단계적 접근을 권고하고 `phase-1-requirements.json`과 `phase-2-requirements.json`을 생성한다. 이 15개 게이트는 **요구사항 층위의 단계 분할 트리거**다 — 설계 층위에서 architect가 적용하는 "라우트 20개 상한"(architect.md 참조)과는 다른 층위이며, 둘은 독립적으로 작동한다.
...
2. **Write**: `requirements.json` — 크면 스켈레톤 먼저 Write 후 각 배열을 Edit로 채운다. P0 FR이 15개 초과 시 `phase-1-requirements.json` / `phase-2-requirements.json`으로 분할.
3. **Write**: `requirements.md` — 한국어 마크다운
  ...
  "key_decisions": [
    {
      "id": "D1",
      "title": "<확정 결정 요약>",
      "decision": "<무엇이 확정되었나 — 원문 그대로>",
      "category": "compute | hosting | auth | data | integration | agent-comm | policy | observability | deployment | other",
      "status": "confirmed | proposed | open",
      "source": "<brief/req의 출처 위치, 예: req.md §0 D1>",
      "related_frs": ["FR-021"]
    }
  ],
...
> **`key_decisions[]` (확정 결정 보존 — 필수)**: 고객이 brief/req에서 **확정한 기술 결정**(예: "에이전트는 AgentCore Runtime에서 호스팅", "에이전트 간 통신은 A2A", "IdP는 Cognito", "외부 시스템 접근은 MCP로 표준화")을 빠짐없이 추출한다. brief/req에 "확정", "Key Decisions", "결정 사항", D-table 같은 표가 있으면 각 행을 1개 `key_decisions[]` 항목으로 만든다. **확정 결정을 "기능 서술"로만 희석하지 마라** — "오케스트레이터가 도메인 에이전트를 호출한다"는 기능 서술이지만, "A2A 프로토콜로 호출한다"는 보존해야 할 아키텍처 제약이다. `status: "confirmed"`인 항목은 다운스트림(architect)이 disposition을 반드시 기록해야 하며, `check-decision-preservation.mjs`(sub-check [O])가 이를 강제한다. 확정 결정이 없으면 `[]`로 둔다(검사는 vacuous PASS).
