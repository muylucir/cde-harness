...
`ai-contract.json`의 `sse_events[]` 중 **모델이 프롬프트 출력 본문에 직접 쓰는 섹션 마커**로 표현되는 이벤트(예: `health_score`→`HEALTH_SCORE:`, `recommendation`→`RECOMMENDATION:`)와 `ai-internals.json`의 `system_prompt` 마커는 **동일한 루트**에서 파생되어야 한다.

- 이벤트명 snake_case → 마커는 `SCREAMING_SNAKE_CASE:` (자동 매핑 가능한 관계)
- `ai-contract.json`에 `section_marker_map`(event_type → marker) 필드를 포함한다. 예: `{ "recommendation": "RECOMMENDATION" }`

> **⚠️ `section_marker_map`에는 "프롬프트 마커 이벤트"만 넣는다 — transport 이벤트는 넣지 않는다.**
> 네이티브 Strands 스트리밍에서 `text`(토큰 델타), `tool_start`/`tool_end`, `done`은 **SDK transport 이벤트**다 — 모델이 텍스트 마커로 출력하는 게 아니라 SDK/라우트가 스트림 프레임으로 내보낸다. 이들은 `sse_events`에는 있지만 프롬프트 마커가 **없으므로** `section_marker_map`에 **넣으면 안 된다**(넣으면 Check 5가 "프롬프트에 마커 없음"으로 FAIL).
> 마찬가지로 `error`는 `sse_events`가 아니라 `error_events` 소속이므로 `section_marker_map` key가 될 수 없다(Check 5b FAIL).
> 즉 단순 챗/추천처럼 모델 출력이 자유 텍스트 + 마커 0~1개면 `section_marker_map`은 비거나 마커 이벤트만 담는다(예: `{ "recommendation": "RECOMMENDATION" }`).

### 양방향 매칭 검증 (필수)

ai-contract.json과 ai-internals.json을 모두 Write한 직후, 다음 두 방향 모두 검증한다 (검증 대상 = **프롬프트 마커 이벤트만**, transport 이벤트 제외):

1. **marker → prompt**: `section_marker_map`의 모든 value(마커 문자열)가 `ai-internals.json.system_prompt.template`(또는 `system_prompts[].template`) 본문에 등장해야 한다. (Check 5)
2. **prompt marker → map**: 시스템 프롬프트 본문에서 `^[A-Z][A-Z0-9_]+:` 패턴으로 추출한 모든 마커가 `section_marker_map`의 value로 존재해야 한다(역색인). 모델이 출력하는 마커가 SSE 이벤트로 매핑되지 않으면 FE가 무시하게 된다.
3. **map key ⊆ sse_events**: `section_marker_map`의 모든 key가 `sse_events[].event_type`에 존재해야 한다(존재하지 않는 이벤트나 error_events 소속을 가리키면 FAIL). transport 이벤트는 맵에 없어도 된다. (Check 5b)
...
- [ ] **프롬프트 `<output_format>`의 섹션 마커가 `section_marker_map`의 값과 1:1 일치하는가** (transport 이벤트 text/tool_*/done 제외 — 그건 맵에 없어야 함)
- [ ] **`section_marker_map`의 key가 모두 `sse_events[].event_type`에 존재하는가** (key ⊆ sse_events; transport 이벤트는 맵에서 제외, error는 error_events 소속이라 맵 key 불가)
