...
> **⚠️ `section_marker_map`에는 "모델이 프롬프트 본문에 직접 출력하는 마커" 이벤트만 넣는다 — transport 이벤트는 넣지 않는다.**
> 네이티브 Strands 스트리밍에서 `text`(토큰 델타), `tool_start`/`tool_end`, `done`은 **SDK transport 이벤트**다 — 모델이 텍스트 마커로 출력하는 게 아니라 SDK/라우트가 스트림 프레임으로 내보낸다(라우트가 emit하므로 Check 4가 별도 보장). 이들은 `sse_events`에는 있지만 프롬프트 마커가 없으므로 `section_marker_map`에 **넣지 않는다**.
> 즉 단순 챗/추천처럼 모델 출력이 자유 텍스트 + 마커 0~1개면 `section_marker_map`은 비거나 마커 이벤트만 담는다(예: `{ "recommendation": "RECOMMENDATION" }`).
> 모델이 출력하는 마커임을 명시하고 싶으면 `ai-internals.json.system_prompt.marker_emitted_by_model: ["RECOMMENDATION"]`로 선언한다 — Check 5가 이 값이 프롬프트에 실재하는지 강제한다(선택 필드).

### 양방향 매칭 검증 (필수, transport-aware)

ai-contract.json과 ai-internals.json을 모두 Write한 직후, 다음을 검증한다 (검증 대상 = **모델 출력 마커만**, transport 이벤트 제외):

1. **prompt marker → map.values (Check 5)**: 시스템 프롬프트 본문에서 `^[A-Z][A-Z0-9_]+:` 패턴으로 추출한 모든 마커가 `section_marker_map`의 value에 존재해야 한다. 모델이 출력하는 마커가 SSE 이벤트로 매핑되지 않으면 FE가 무시하게 되므로 차단. (+ `marker_emitted_by_model`로 선언한 마커는 프롬프트에 실재해야 함.)
2. **map key ⊆ (sse_events ∪ error_events) (Check 5b)**: `section_marker_map`의 모든 key가 `sse_events[].event_type` 또는 `error_events[].event_type`에 존재해야 한다(어느 쪽에도 없는 고아 key면 FAIL). transport 이벤트는 맵에 없어도 정상.
...
- [ ] **프롬프트가 출력하는 ALL_CAPS 마커가 모두 `section_marker_map`의 값에 있는가** (Check 5; transport 이벤트 text/tool_*/done은 모델 마커가 아니므로 제외)
- [ ] **`section_marker_map`의 key가 모두 `sse_events` 또는 `error_events`의 event_type인가** (Check 5b, key ⊆ sse ∪ error; transport 이벤트는 맵에서 제외)
