...
3. **CHECKPOINT를 통과해야 완료 처리한다** — README/HANDOVER 외에도 환경변수 정합성·AI 모킹 부재·시크릿 노출 부재를 검증한다.
...
생성 문서 (handover-packager 책임):
- `README.md` — 빌드/실행/테스트 방법, 환경변수 가이드
- `HANDOVER.md` — 인수인계 요약, 의사결정 기록, 권장 다음 단계
- `docs/ARCHITECTURE.md` — 아키텍처 개요
- `docs/API.md` — API 라우트 카탈로그
- `docs/AI-AGENT.md` (AI 기능 있을 때) — Strands SDK 구조, 모델 선택 근거, 도구 카탈로그
- `docs/AWS.md` (/awsarch 실행됐을 때) — CDK 스택, 리소스, 비용 추정
- `docs/CHECKLIST.md` — 프로덕션 전환 체크리스트
...
node .pipeline/scripts/checkpoint.mjs check handover-packager \
  ...
  "file:.pipeline/artifacts/v{N}/07-handover/HANDOVER.md" \
...
- [ ] **`docs/CHECKLIST.md`에 AI mocking 부재 / `@aws-sdk/client-bedrock-runtime` 미사용 명시**
...
1. Launch `git-manager` agent with action: `post-handover`
   - 핸드오버 문서(docs/, README.md, HANDOVER.md, .env.local.example) 자동 커밋
