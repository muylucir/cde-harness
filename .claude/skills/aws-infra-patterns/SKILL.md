...
### Step 1: 프로토타입 단계 vs 프로덕션 단계

| 단계 | 기본 선택 | 이유 |
|---|---|---|
| **CDE 프로토타입 (`/pipeline`)** | **Next.js 16 App Router Route Handler** (SSE 스트리밍) | mock 모드. 별도 배포 불필요, 로컬에서 바로 동작. AgentCore 비용 발생 안 함 |
| **AWS 전환 (`/awsarch`)** | 아래 트리 따라 분기 | 실제 인프라로 승격할 때만 결정 |

> **중요**: CDE 파이프라인의 `/pipeline` 단계에서는 **항상 Next.js Route Handler로 SSE 스트리밍**을 구현한다 (`strands-sdk-typescript-guide`의 SSE 패턴 참조). AgentCore Runtime이나 Lambda 배포는 `/awsarch` 시 결정된다. code-generator-ai는 이 단계에서 AgentCore 직접 배포 코드를 만들지 않는다.

### Step 2: AWS 전환 시 의사결정 트리 (`/awsarch` 단계)

```
AI 에이전트 복잡도 분석 시작 (/awsarch 단계)
    │
    ├─ 챗 UI (SSE 스트리밍)이 핵심?  ← 가장 흔한 CDE 케이스
    │   ├─ YES + 단순 (1-2개 도구, 무상태) → **Next.js Route Handler 유지** (App Runner / ECS / Vercel 배포)
    │   ├─ YES + 멀티턴/메모리 필요 → **AgentCore Runtime** (Strands BedrockAgentCoreApp 래핑)
    │   └─ NO ↓
    │
...
**기본 원칙**:
- **CDE 프로토타입에서는 Next.js Route Handler가 default** (mock 모드). AgentCore/Lambda는 `/awsarch`에서 결정.
...
| 스킬 | 담당 영역 |
|---|---|
| **aws-infra-patterns** (이 문서) | 어떤 AWS 서비스를 선택할지 (의사결정) |
| **strands-sdk-typescript-guide** | Next.js Route Handler에서 Strands Agent 코드 작성 (구현) |
| **bedrock-agentcore-guide** | AgentCore Runtime/Memory/Gateway 사용법 (운영) |
| **aws-cdk-patterns** | 위 결정을 CDK TypeScript로 구현 (인프라 코드) |
