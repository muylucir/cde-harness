...
description: >
  AWS 인프라를 "어떤 서비스로 설계할지" 결정할 때 반드시 호출. 스토리지 선택(DynamoDB/Aurora/ElastiCache/
  OpenSearch), 통신·이벤트·비동기 레이어(SQS/SNS/EventBridge/Step Functions/Lambda/Kinesis),
  AI 에이전트 런타임(Lambda+Strands vs AgentCore) 의사결정 트리, 서비스별 설계 고려사항,
  IAM 정책 템플릿, 비용 추정 공식을 제공한다.
...
- **모드**: Serverless (프로토타입 최적). 엔진은 **Valkey 권장** — Redis OSS와 API 호환이며 Serverless 기준 ~33% 저렴
- **캐시 전략**: Cache-Aside (개별 엔티티, TTL 5-15분), 대시보드 집계 (TTL 1-5분), 세션 (TTL 30분)
- **보조 역할**: 기본 스토리지(DynamoDB/Aurora) 앞에 캐시 레이어로 배치
- **주의**: 캐시 무효화 전략 필수 (write-through 또는 TTL 기반). 클라이언트는 기존 Redis 라이브러리 그대로 사용
...
- **런타임**: Node.js 22 (`NODEJS_22_X`, 최신 LTS — Next.js와 일관). Node 20은 노후화 중
...
> [!IMPORTANT]
> 배포는 `@aws/agentcore` **npm CLI**(`agentcore create`/`deploy`, CDK 기반)로 한다 — 예전 `agentcore configure`/`launch --mode codebuild` 흐름은 deprecated. 에이전트 코드는 `from bedrock_agentcore.runtime import BedrockAgentCoreApp`. 메모리 API는 `StoreMemory`/`RetrieveMemory`가 아니라 데이터면의 `CreateEvent`/`RetrieveMemoryRecords` 등이다.

- **Runtime**: Strands 등 에이전트를 `BedrockAgentCoreApp`으로 래핑 → `agentcore deploy`로 서버리스 배포. VPC 불필요, 세션 관리 내장
- **Memory**: STM(세션 내 이벤트/대화) + LTM(전략 기반 추출: SEMANTIC/SUMMARIZATION 등). 멀티턴 챗봇에 필수
...
| AgentCore (호출) | bedrock-agentcore:InvokeAgentRuntime, GetAgentRuntime | `runtime/${agentId}` |
| AgentCore (메모리·데이터면) | bedrock-agentcore:CreateEvent, RetrieveMemoryRecords, ListEvents | `memory/${memoryId}` |
...
| Bedrock | `BEDROCK_MODEL_ID` (예: `global.anthropic.claude-sonnet-4-6`) |
| AgentCore | `AGENTCORE_RUNTIME_ARN`, `AGENTCORE_MEMORY_ID`, `AGENTCORE_GATEWAY_ID` |
| 공통 | `AWS_REGION` |
