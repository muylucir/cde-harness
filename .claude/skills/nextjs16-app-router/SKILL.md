...
6. **보호 라우트 가드는 `src/proxy.ts`** (Next.js 16에서 `middleware.ts`가 deprecated되고 `proxy.ts`로 리네이밍됨). 상세 인증 패턴은 `nextjs-auth-patterns` 스킬.
...
src/proxy.ts                # 보호 라우트 가드 + 보안 헤더 (구 middleware.ts, Next.js 16에서 리네이밍)
```
...
## Proxy (구 Middleware) — Next.js 16 리네이밍

Next.js 16에서 `middleware.ts` 파일 컨벤션은 **deprecated**되고 **`proxy.ts`**로 리네이밍됐다. CDE 프로토타입은 새 컨벤션만 사용한다.

| 항목 | 구 (deprecated) | 신 (Next.js 16+) |
|---|---|---|
| 파일 경로 | `src/middleware.ts` | `src/proxy.ts` |
| export 함수 | `export function middleware(request) {}` | `export function proxy(request) {}` |
| `config.matcher` | 동일 | 동일 |
| Edge Runtime (기본) | 동일 | 동일 |
| `NextRequest`/`NextResponse` API | 동일 | 동일 |

**기존 코드 마이그레이션 (codemod)**:

```bash
npx @next/codemod@canary middleware-to-proxy .
```

코드모드가 파일명·함수명을 자동 변환한다. config·import는 변동 없음.

**왜 바뀌었나**: Express.js middleware와 혼동을 줄이고, "네트워크 경계 = 마지막 수단"이라는 의미를 부각하기 위함. Next.js 팀은 가능한 경우 Server Component / Route Handler / `generateMetadata`로 동일 목적을 달성하는 것을 권장한다.

**상세 인증 패턴**: `nextjs-auth-patterns` 스킬 참조 (`src/proxy.ts`로 Cognito JWT 검증, 보안 헤더, 401 응답 envelope 처리).
...
| `src/middleware.ts` + `export function middleware()` | `src/proxy.ts` + `export function proxy()` (`npx @next/codemod@canary middleware-to-proxy .`로 마이그레이션) |
...
- [ ] 보호 라우트/보안 헤더 가드가 `src/middleware.ts`가 아닌 `src/proxy.ts`로 작성됐는가 (Next.js 16 컨벤션)
