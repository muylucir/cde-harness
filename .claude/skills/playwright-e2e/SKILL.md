...
4. **로컬 미러 기동 후 실행 (Vision B, Rule 12)**: E2E는 `npm run infra:local`(ministack :4566 + compose Postgres)을 먼저 띄운 뒤 로컬 endpoint env로 돈다. 폐기된 `DATA_SOURCE=memory` 런타임 분기를 쓰지 않는다 — 코드는 실 AWS와 동일하고 endpoint env만 로컬을 가리킨다. 실 AWS 호출은 endpoint가 ministack(:4566)이라 자연히 차단된다.
...
export default defineConfig({
  ...
  webServer: {
    // 로컬 미러(ministack :4566 + compose Postgres)를 먼저 띄운 뒤 dev 서버 기동 (Vision B).
    // 사전 1회: `npm run infra:local`. 코드는 실 AWS와 동일, endpoint env만 로컬(.env.local의 AWS_ENDPOINT_URL/DATABASE_URL).
    command: 'npm run dev',
...
> **기동 순서**: `npm run infra:local`(ministack/compose + cdklocal 시드)을 먼저 실행한 뒤 `npm run test:e2e`. 폐기된 `DATA_SOURCE=memory` 런타임 분기는 쓰지 않는다 (Rule 12, endpoint-only).
...
## 패턴 1b: 사용자 여정 (멀티 페이지 흐름)

`requirements.json`의 `user_stories[]` 중 P0 스토리를 **여러 페이지를 연결하는 흐름**으로 검증한다. 패턴 1(개별 라우트 가시성)과 달리 페이지 간 이동·상태 연속성을 본다. `user-journey.spec.ts`에 배치.
...
> FR 테스트가 개별 기능의 계약이라면, 유저스토리 테스트는 **전체 흐름의 계약**이다. 어서션은 약화하지 않는다.
...
- [ ] E2E가 로컬 미러(`npm run infra:local`) endpoint env로 도는가 (폐기된 `DATA_SOURCE=memory` 분기 미사용 — Rule 12)
