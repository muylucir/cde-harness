...
`/brief` 실행 후 입력 자료에 모호한 부분이 있으면 `.pipeline/input/clarifications.md` 파일이 자동 생성됩니다.
...
**답변 처리:**
- 각 질문의 `답변:` 란에 답변을 작성
- 모르는 항목은 비워두면 추론값이 유지됨
- 답변 후 `/brief`를 다시 실행하거나, `/pipeline` 시작 시 자동 반영
- 반영된 항목에는 `✅ 반영됨` 표시가 추가됨

**질문이 없으면** 이 단계를 건너뛰고 바로 `/pipeline`을 실행하면 됩니다.
